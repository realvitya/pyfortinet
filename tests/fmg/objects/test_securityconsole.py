...
class TestObjectsOnLab:
    def test_install_device(self, fmg):
        ...
        test_device.desc = (test_device.desc + "test: " + str(random.randint(1, 1000))
        ...
        task: InstallDeviceTask = fmg.get_obj(InstallDeviceTask,
                                              adom=fmg.adom,
                                              scope=[Scope(name=test_device.name, vdom=vdom)],
                                              flags=["auto_lock_ws"]
                                              )
