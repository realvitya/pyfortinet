...
def get_fmg_error(url: str, error_code: Union[str, int]) -> Optional[Union[Type[FMGException], str]]:
    """FMG API error code lookup

    Args:
        url (str): FMG API request URL
    ...
    Examples:
        url = "/dvmdb/adom/root"
        error_code = 11
        returns: FMGLockNeededException
    """
    error_code = abs(int(error_code))  # standardize error code to positive int
    # First, we need to test if handling common errors are enough!
    # TODO: cleanup or leave this block
    # check_url = copy(url)
    # lookup_table = {}
    # # remove scope to standardize request url ('global' or '/adom/someadom')
    # # + "/" is there to start searching with full url
    # check_url = re.sub(r"/(global\b|adom/[^/]*)", "", check_url) + "/"
    # while check_url := check_url.rsplit("/", 1)[0]:
    #     if check_url in URL_TO_MODULE_MAPPING:
    #         lookup_table = URL_TO_MODULE_MAPPING[check_url]
    # if lookup_table:
    #     return lookup_table.get(error_code)

    if error_code < len(API_ERRORS["common"]):  # common error messages (these are to be overriden by specific tables)
        return API_ERRORS["common"][error_code]


URL_TO_MODULE_MAPPING = {
    "/dvmdb": "dvmdb",
    "/pm/config/obj": "adomobj",
    "/pm/config/device": "devobj",
    "/pm/config": "cdb",
    "/pm/pkg": "pkg",
    "/pm/pblock": "pkg",
    "/pm/config/pkg/": "pkg",
    "/pm/wanprof": "wanprof",
    "/pm/config/wanprof": "wanprof",
    "/pm/devprof": "devprof",
    "/pm/config/devprof": "devprof",
    "/cli": "cmdb",
    "/dvm/cmd": "dvmcmd",
}
